  ...
  return (
    <div className="flex justify-center ">
      <div className="flex flex-col w-7/12 gap-4 bg-gray-100">
        <h1 className="text-4xl font-bold m-4 p-4  h-15 bg-white">{menu[0].card.card.text}</h1>
        {menu[5].groupedCard.cardGroupMap.REGULAR.cards.map((item) => {
          return item.card.card.itemCards && <RestrauntMenu items={item.card.card} key={item.card.card.categoryId}/>
